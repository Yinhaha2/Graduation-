    ...
    *,
    ...
    try:
        ...
        meta_path.write_text(
            json.dumps({"usage": usage, "cost": cost, "model": model, "output": str(out)}, indent=2),
            encoding="utf-8",
        )
        print(f"Wrote {out}")
        if usage:
        ...
        raise RuntimeError(f"Parse failed; raw saved to {raw_backup}") from exc


def pick_batch_pr_ids(count: int, *, skip_analyzed: bool = True) -> list[int]:
    master = pd.read_csv(MASTER_CSV)
    skip = set(FEWSHOT_PR_IDS)
    picked: list[int] = []
    for pid in master["id"].astype(int):
        if pid in skip:
            continue
        if skip_analyzed:
            if output_path(pid).exists():
                continue
        picked.append(pid)
        if len(picked) >= count:
            break
...
def run_batch(
    ...
    *,
    ...
    report_path = report_path or MYSTUDY / "output_pr_analysis" / "batch_run_report.json"
    md_path = MYSTUDY / "output_pr_analysis" / "batch_run_report.md"
    ...
    for i, pr_id in enumerate(pr_ids, 1):
        ...
        try:
            ...
            rows.append({"pr_id": pr_id, "ok": False, "error": str(exc), "batch_index": batch_index})
...
def main() -> None:
    ...
    run_analysis(args.pr_id, dry_run=args.dry_run, model=args.model, save_prompt=args.save_prompt)
